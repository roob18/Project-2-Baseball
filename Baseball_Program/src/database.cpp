...
QSqlQuery Database::getTeamInfo(const QString &teamName)
{
    ...
    query.prepare(
        ...
        "CASE "
        "  WHEN center_field > 0 THEN center_field || ' feet (' || CAST(ROUND(center_field * 0.3048) AS INTEGER) || 'm)' "
        "  ELSE '0' "
        "END as center_field, "
        "typology, roof "
...
QSqlQuery Database::getTeamsWithGreatestCenterField()
{
    ...
    if (!query.exec(
        ...
        "       center_field || ' feet (' || CAST(ROUND(center_field * 0.3048) AS INTEGER) || 'm)' as center_field "
        "FROM teams "
...
bool Database::importFromCSV(const QString &filename)
{
    ...
    while (!in.atEnd()) {
        ...
        if (fields.size() >= 10) {
            ...
            centerFieldStr.remove(QRegularExpression("[^0-9]"));
            int centerField = centerFieldStr.toInt();
