...
void MainWindow::displayOpenRoofTeams()
{
    ...
    // Count teams with open roof first
    int count = 0;
    while (query.next()) {
        count++;
    }
    
    // Reset the query for display
    query = db->getOpenRoofTeams();
    displayQueryResults(query, headers);
    
    QMessageBox::information(this, "Open Roof Count",
                           QString("Number of teams with open roof: %1").arg(count));
}
...
void MainWindow::displayTeamsByCapacity()
{
    ...
    int totalCapacity = 0;
    while (query.next()) {
        totalCapacity += query.value(2).toInt();
    }
    
    // Reset the query for display
    query = db->getTeamsByCapacity();
    displayQueryResults(query, headers);
    
    QMessageBox::information(this, "Total Capacity",
                           QString("Total MLB seating capacity: %1").arg(totalCapacity));
}
...
void MainWindow::onImportCSV()
{
    ...
    if (db->importFromCSV(fileName)) {
        ...
        // Refresh your views/tables here if needed
    } else {
